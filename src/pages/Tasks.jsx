  ...
  return (
    <div className='w-full'>
      <div className="head max-sm:flex-col flex justify-between items-center">
        <h1>You have total <strong>{tasks?.length}</strong> tasks</h1>
        <div className='flex relative w-full justify-center'>
          <div className="taskSearch flex items-center">
            <SearchBar handler={search}/>
          </div>
          {getAuthInfo().isAdmin?
          <button onClick={()=>setAddTask(true)} className='p-2 absolute right-0 font-bold bg-[#4285F4] rounded text-white hover:opacity-70'>Add task</button>:null}
        </div>
      </div>
        ...
        <table className='table-auto max-sm:hidden max-sm:text-xs w-full' >
          <thead className='static'>
            <tr className='bg-blue-100'>
              ...
              <th className='border-2 border-slate-400  p-2 w-[150px]'>Deadline</th>
              <th className='border-2 border-slate-400  p-2 w-[150px]'>Status</th>
              <th className='border-2 border-slate-400  p-2 w-[150px]'>Created By</th>
            </tr>
          </thead>
          <tbody>
            {tasks?.map((item)=>{
