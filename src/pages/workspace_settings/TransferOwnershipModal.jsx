...
const TransferOwnershipModal = ({ setModal }) => {
  ...
  const { values, errors, touched, handleBlur, handleChange, handleSubmit, resetForm, setFieldValue } = useFormik({
    ...
    onSubmit: async (data) => {
      ...
      try {
        const res = await postData("/requests", { workspaceId, targetUserId, type });
    ...
    <Dialog open={true} onOpenChange={setModal}>
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <Textarea value={values.message} name="message" onChange={handleChange} onBlur={handleBlur} placeholder="Message (optional)" label="Message" />
            {errors.message && touched.message && <div className="text-red-500 text-sm">{errors.message}</div>}
          </div>
          <DialogFooter className="flex gap-4">
            <Button disabled={loading} type="submit" className="bg-blue-600 w-full">Transfer {loading && <Loader />}</Button>
            <Button variant="outline" onClick={()=>setModal(false)} className="w-full">Cancel</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
